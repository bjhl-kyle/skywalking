...
public class ProfileClient extends SimpleQueryClient {
    ...
    public ProfileTaskCreationResult createProfileTask(
        final ProfileTaskCreationRequest creationRequest) throws Exception {
        ...
        final ResponseEntity<GQLResponse<ProfileTaskCreationResultWrapper>> responseEntity = restTemplate.exchange(new RequestEntity<>(queryString, HttpMethod.POST, URI
            .create(endpointUrl)), new ParameterizedTypeReference<GQLResponse<ProfileTaskCreationResultWrapper>>() {
        });
    ...
    public ProfileTasks getProfileTaskList(final ProfileTaskQuery query) throws IOException {
        ...
        final ResponseEntity<GQLResponse<ProfileTasks>> responseEntity = restTemplate.exchange(new RequestEntity<>(queryString, HttpMethod.POST, URI
            .create(endpointUrl)), new ParameterizedTypeReference<GQLResponse<ProfileTasks>>() {
        });
    ...
    public List<Trace> getProfiledTraces(final String taskId) throws Exception {
        ...
        final ResponseEntity<GQLResponse<Traces>> responseEntity = restTemplate.exchange(new RequestEntity<>(queryString, HttpMethod.POST, URI
            .create(endpointUrl)), new ParameterizedTypeReference<GQLResponse<Traces>>() {
        });
