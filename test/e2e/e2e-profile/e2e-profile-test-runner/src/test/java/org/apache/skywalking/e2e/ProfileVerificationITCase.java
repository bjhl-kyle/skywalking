...
public class ProfileVerificationITCase {
    ...
    public void verify() throws Exception {
        ...
        while (true) {
            try {
                ...
                final List<Trace> traces = profileClient.traces(new TracesQuery().start(minutesAgo)
    ...
    private void verifyProfiledSegment(String taskId) throws InterruptedException {
        ...
        for (int i = 0; i < 10; i++) {
            try {
                ...
                InputStream expectedInputStream = new ClassPathResource("expected-data/org.apache.skywalking.e2e.ProfileVerificationITCase.profileSegments.yml")
                    .getInputStream();
    ...
    private void verifyProfileTask(int serviceId, String verifyResources) throws InterruptedException {
        ...
        for (int i = 0; i < 10; i++) {
            try {
                final ProfileTasks tasks = profileClient.getProfileTaskList(new ProfileTaskQuery().serviceId(serviceId)
                                                                                                  .endpointName(""));
    ...
    private void verifyServices(LocalDateTime minutesAgo) throws Exception {
        ...
        InputStream expectedInputStream = new ClassPathResource("expected-data/org.apache.skywalking.e2e.ProfileVerificationITCase.services.yml")
            .getInputStream();
    ...
    private Instances verifyServiceInstances(LocalDateTime minutesAgo, LocalDateTime now,
        ...
        Instances instances = profileClient.instances(new InstancesQuery().serviceId(service.getKey())
                                                                          .start(minutesAgo)
                                                                          .end(now));
        LOGGER.info("instances: {}", instances);
        expectedInputStream = new ClassPathResource("expected-data/org.apache.skywalking.e2e.ProfileVerificationITCase.instances.yml")
            .getInputStream();
    ...
    private Endpoints verifyServiceEndpoints(LocalDateTime minutesAgo, LocalDateTime now,
        ...
        LOGGER.info("instances: {}", instances);
        InputStream expectedInputStream = new ClassPathResource("expected-data/org.apache.skywalking.e2e.ProfileVerificationITCase.endpoints.yml")
            .getInputStream();
